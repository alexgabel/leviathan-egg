#!/usr/bin/env python3
"""Run Phase 4 configs listed in a manifest via the project CLI runner."""
...
def _write_override_config(
    *,
    ...
    ge = cfg.get("graph_extraction", {}) or {}
    if bool(ge.get("enabled", False)):
        window_periods = int(ge.get("window_periods", 1))
        seasonal_period = int(cfg["seasonality"]["seasonal_period"])
        required_post_burn = window_periods * seasonal_period
        actual_post_burn = int(total_steps) - int(burn_in)
        if actual_post_burn < required_post_burn:
            raise ValueError(
                "Smoke override too short for Phase-4 graph extraction: "
                f"post-burn steps={actual_post_burn}, required>={required_post_burn} "
                f"(window_periods={window_periods}, seasonal_period={seasonal_period})"
            )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run Phase 4 configs from manifest")
    parser.add_argument(
        "--manifest",
        type=Path,
    ...
    parser.add_argument("--stage", type=str, default="exploration")
